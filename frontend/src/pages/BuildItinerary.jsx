...
import { Plus, MapPin, Calendar, DollarSign, Trash2, GripVertical, Plane, Hotel, Navigation, Search, Loader } from 'lucide-react';
...
const BuildItinerary = () => {
  ...
  // Parse tripId from URL query parameters
    ...
    if (tripId) {
      const fetchTripDetails = async () => {
        ...
        try {
          ...
          if (!token) {
            setError('Please login to build your itinerary.');
            return;
          }
  ...
  const handleSaveItinerary = async () => {
    if (!tripId) {
      setError('No active trip reference. Please plan a trip first.');
      return;
    }
    setSaveLoading(true);
    setError('');
    ...
    try {
      ...
      // We will loop and create a Stop and an Activity for each itinerary section
      for (let i = 0; i < sections.length; i++) {
        const section = sections[i];

        // 1. Create a Stop for this section
        const stopRes = await fetch('/api/stops', {
          ...
          body: JSON.stringify({
            tripId: tripId,
            city: trip ? trip.tripName.replace('Trip to ', '') : 'Destination',
            country: 'Destination',
            arrivalDate: section.startDate || undefined,
            departureDate: section.endDate || undefined,
        ...
        if (stopData.success) {
          const stopId = stopData.data.stop._id;

          // Category mapping
          let category = 'sightseeing';
          if (section.type === 'flight') category = 'transport';
          if (section.type === 'hotel') category = 'accommodation';

          // Budget parsing
          let cost = 0;
          if (section.budget) {
            const numericBudget = section.budget.replace(/[^0-9.]/g, '');
            cost = parseFloat(numericBudget) || 0;
          }

          // 2. Create the corresponding Activity under this Stop
      ...
      setError('An error occurred while saving the itinerary stops.');
    } finally {
      setSaveLoading(false);
    }
  };
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 relative z-10">
        ...
        >
          <h1 className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-primary-400 to-indigo-400 mb-4">
            {trip ? `Itinerary for ${trip.tripName}` : 'Build Your Itinerary'}
          </h1>
          <p className="text-gray-400 text-lg max-w-2xl mx-auto">
            {trip 
              ? `Organize your adventure from ${trip.startDate ? new Date(trip.startDate).toLocaleDateString() : 'soon'} to ${trip.endDate ? new Date(trip.endDate).toLocaleDateString() : 'whenever'}.`
              : 'Organize your trip day by day. Add flights, hotels, and activities to create your perfect schedule.'
            }
          </p>
          {error && (
            <p className="text-red-400 font-semibold mt-4 text-center">{error}</p>
          )}
        </motion.div>
        ...
        >
          <button 
            ...
            className="bg-gradient-to-r from-primary-600 to-indigo-600 hover:from-primary-500 hover:to-indigo-500 text-white px-10 py-4 rounded-full font-bold shadow-[0_10px_40px_rgba(79,70,229,0.4)] transition-all transform hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
          >
            {saveLoading ? (
              <>
